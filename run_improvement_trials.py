...
from src.data import load_dataset, ALL_DATASETS, MNIST_DATASETS, SLOW_DATASETS
from src.experiments import performance_improvement_trial
from src.utils import create_folder, StoreStringList, StoreFloatList


def _main_function_improvement_exp(
        datasets: list[str],
    ...
    for ds_name in datasets:
        if ds_name not in ALL_DATASETS + MNIST_DATASETS:
            ...
            for random_state in range(seed, seed+n_trials):
                for ds_name in datasets:
                    ...
                    percentiles = np.linspace(
                        0.0, noise_level + 0.05, interval_steps+1
                    )
                    _trial_params = deepcopy(base_trial_params)
                    _trial_params['percentiles'] = percentiles.tolist()
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '-ds', '--datasets', default=["digits", "spirals", "satelite"], 
        type=str, action=StoreStringList, help="Datasets to use"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        '-nl', '--noise_levels', default=[0.0, 0.05, 0.1], type=float,
        action=StoreFloatList, help="Noise levels to use"
    )
    parser.add_argument(
        '-nt', '--noise_types', default=['uniform', 'asymmetric'], 
        action=StoreStringList, help="Noise types to use"
    )
    parser.add_argument(
        '-i', '--interval_steps', default=20, type=int, 
        help="Percentile interval steps for removing mislabelled samples"
    )
    parser.add_argument(
    ...
    # Run the main function for the performance improvement experiments
    results = _main_function_improvement_exp(
    ...
    path = os.path.join(args.save_results, 'improvement_trials.csv')
    results.to_csv(path)

    print(f"Improvement trials results saved at {path}")